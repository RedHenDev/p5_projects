...
function preload(){
    ...
    mapImage = loadImage("https://api.mapbox.com/styles/v1/mapbox/light-v9/static/0,0," + ziddy + ",0,0/" + widdy + "x" + hiddy + "?access_token=pk.eyJ1IjoicmVkaGVuZGV2IiwiYSI6ImNpemJzMzQyczAwM2QycW95aWE3MW9wd3EifQ.cES0OnSEDQBjKSriBe7T-w");
...
function setup(){
    ...
    quantumButton.style("background-color", 'blue');
...
function start_attractors(){
    ...
    for (var i = 0; i < csvEQ.length; i++){
        ...
        attractors.push(new attractor(qx[i]+width/2,qy[i]+width/2, magN[i]/10));
    }
...
function renderQuakes(){
    ...
    for (var i = 0; i < csvEQ.length; i++){
        ...
        ellipse (qx[i],qy[i],magN[i]*0.1,magN[i]*0.1);
