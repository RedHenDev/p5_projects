...
function toggle_showParticles(){
    ...
    if (!showParticles) {alphaSlider.value(22); strokeWeight(1); fill(0); background(BGCOLOUR);
               }
};
...
function setupNew(){
   background(BGCOLOUR);
   for (var i = 0; i < 444; i++){ 
       particles.push(new particle(random(0,WIDTH), random(0,HEIGHT),1,1,random(0,255)));
        var newV = createVector(random(-3,3),random(-3,3));
        particles[i].velocity = newV;
        //particles[i].acceleration = newV;
        particles[i].maxV = 2.4;
   }
...
function draw() {
    ...
    for (var i = 0; i < particles.length; i++){ 
        ...
        particles[i].update();
        particles[i].render(alphaSlider.value());
   }
